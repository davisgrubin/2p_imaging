...
def _load_flat_csvs(root: Path) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Load CSVs that sit directly under ``root`` instead of per-session folders."""

    def _maybe_read(filename: str) -> pd.DataFrame:
        path = root / filename
        if not path.exists():
            return pd.DataFrame()
    ...
    return (
        _maybe_read(TIMECOURSE_FILE),
        _maybe_read(WHOLE_FILE),
        _maybe_read(CONFUSION_FILE),
    )
...
class DataRepository:
    ...
    def reload(self) -> DecoderDataset:
        ...
        if timecourse_df.empty and metrics_df.empty and confusion_df.empty:
            timecourse_df, metrics_df, confusion_df = _load_flat_csvs(self.root)
